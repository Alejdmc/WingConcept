...
import { parseListPrice } from '@/lib/loadPartsCatalog'
import { loadAdminParaglidersCatalog } from '@/lib/loadParaglidersCatalog'
...
const CATEGORIES = [
  { value: 'vela', label: 'Wing (vela)' },
  { value: 'accesorios', label: 'Harness (accesorios — slug pgl-harness-*)' },
]

function isHarnessItem(item) {
  return item.categoria === 'accesorios' && String(item.slug || '').startsWith('pgl-harness-')
}

function emptyForm(categoria = 'vela') {
  return {
    id: null,
    varianteId: null,
    nombre: '',
...
function fromProduct(product) {
  ...
  return {
    ...
    categoria: product.categoria || 'vela',
...
function CatalogForm({ initial, onSave, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white border border-borderline rounded-lg p-6 space-y-4">
      ...
      {form.categoria === 'accesorios' && !form.id && (
        <p className="text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded p-3">
          Harness names should start with a clear brand/model. After saving, set slug to <code className="bg-white px-1 rounded">pgl-harness-…</code> via seed or DB if needed for cart ID mapping.
        </p>
      )}
      ...
      <ImageUploadField
        ...
        productoId={form.id}
        label="Product images (up to 3 — shown on /paragliders)"
        maxImages={3}
      />

      <label className="flex items-center gap-2 text-sm">
        <input type="checkbox" name="activo" checked={form.activo} onChange={handleChange} />
        Active (visible on /paragliders and addable to cart)
      </label>
...
export default function AdminParaglidersPage() {
  ...
  const load = async () => {
    ...
    setError('')
    try {
      const rows = await loadAdminParaglidersCatalog()
      const combined = rows
      ...
      try {
        ...
        const catalogIds = new Set(combined.map((i) => i.id))
        const paragliderAlerts = (alertData.items || []).filter((a) => catalogIds.has(a.producto_id))
        setLowStockAlerts(paragliderAlerts)
      } catch {
        setLowStockAlerts([])
      }
    } catch {
  ...
  const saveItem = async (form) => {
    ...
    const variantePayload = {
      ...
      atributos: { standalone_paraglider: true },
    }

    if (form.id) {
      await api.admin.actualizarProducto(form.id, payload)
      if (form.varianteId) {
        await api.admin.actualizarVariante(form.varianteId, variantePayload)
      }
    } else {
  ...
  const handleDelete = async (id) => {
    if (!confirm('Deactivate this item? It will no longer appear on /paragliders or in cart.')) return
    try {
  ...
  const filtered = items.filter((item) => {
    ...
    if (filter === 'vela') return item.categoria === 'vela'
    if (filter === 'harnesses') return isHarnessItem(item)
    return true
  })
  ...
  const defaultFormCategory = filter === 'harnesses' ? 'accesorios' : 'vela'

  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-black text-ink">Paragliders</h1>
          <p className="text-ink2 mt-2">
            Same catalog as <code className="text-xs bg-bg2 px-1 py-0.5 rounded">/paragliders</code> — active items ({activeCount}) appear on the site. Inactive items are hidden from customers.
          </p>
        </div>
        <button
      ...
      {inactiveCount > 0 && (
        <div className="mb-6 p-4 rounded-lg border border-borderline bg-bg2 text-sm text-ink2">
          {inactiveCount} inactive item{inactiveCount !== 1 ? 's' : ''} (dimmed below — not shown on /paragliders).
        </div>
      )}
      ...
      {lowStockAlerts.length > 0 && (
        <div className="mb-6 p-4 rounded-lg border border-orange-300 bg-orange-50 text-sm">
          <p className="font-bold text-orange-900">
            {lowStockAlerts.length} paraglider item{lowStockAlerts.length !== 1 ? 's' : ''} with low stock (≤ {LOW_STOCK_THRESHOLD} units)
          </p>
        </div>
      )}
      ...
      {creating && (
        <div className="mb-8">
          ...
          <CatalogForm initial={emptyForm(defaultFormCategory)} onSave={saveItem} onCancel={() => setCreating(false)} />
        </div>
      )}
        ...
        <table className="w-full min-w-[800px] text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan="8" className="py-8 text-center text-ink2">No items in this category. Run seed_paragliders_catalog.py in the backend to populate the catalog.</td></tr>
            ) : (
              filtered.map((item, index) => {
                const variante = item.variantes?.find((v) => v.es_principal) || item.variantes?.[0]
                const stock = variante?.stock ?? 0
                const isLowStock = stock > 0 && stock <= LOW_STOCK_THRESHOLD
                const typeLabel = item.categoria === 'vela' ? 'Wing' : isHarnessItem(item) ? 'Harness' : item.categoria
                return (
                  <tr key={item.id} className={`border-t border-borderline hover:bg-bg2 ${!item.activo ? 'opacity-60' : ''} ${isLowStock ? 'bg-orange-50/60' : ''}`}>
                    ...
                    <td className="py-4 px-6 capitalize">{typeLabel}</td>
