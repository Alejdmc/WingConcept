...
import { DISRUPTOR_PARAMOTOR_HERO } from '@/lib/disruptorParamotorContent'
...
function fromProduct(p) {
  ...
  return {
    ...
    precio,
    listing: {
      tagline: listing.tagline || p.contenido_extra?.tagline || '',
      description: listing.description || p.descripcion_corta || '',
      featuresText: (listing.features || []).join('\n'),
      image: listing.image || p.imagenes?.[0] || (p.slug === 'disruptor-paramotor' ? DISRUPTOR_PARAMOTOR_HERO : ''),
...
export default function AdminParamotorsPage() {
  ...
  const load = async () => {
    ...
    setError('')
    try {
      const data = await api.admin.productos({ categoria: 'paramotor', por_pagina: 50 })
      const ids = (data.items || [])
        .filter((p) => PARAMOTOR_HREFS[p.slug])
        .map((p) => p.id)
      ...
      setError('Could not load paramotors.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const saveItem = async (item) => {
    ...
    try {
      ...
      const contenido_extra = {
        ...
        tagline: item.listing.tagline || null,
        listing: {
      ...
      setMessage(`"${item.nombre}" saved. Changes appear on /paramotors.`)
    } catch (err) {
  ...
  return (
    <div>
      <div className="mb-8 flex flex-wrap items-start justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-ink">Paramotors page</h1>
          <p className="text-ink2 mt-2 max-w-2xl">
            Edit selection cards on <strong>/paramotors</strong> (Disruptor Paramotor).
            Banner texts use <Link href="/admin/site" className="text-brand hover:underline">Website texts</Link> when paramotor blocks are seeded.
            Configurator prices: <Link href="/admin/configurador" className="text-brand hover:underline">Configurators</Link>.
          </p>
        </div>
        <a
          href="/paramotors"
      ...
      {loading ? (
        ...
        <p className="text-ink2">No paramotor products found with a public page mapping.</p>
      ) : (
          ...
          {items.map((item) => (
            <div key={item.id} className={`bg-white border rounded-xl p-6 ${!item.activo ? 'opacity-60' : ''}`}>
              <div className="flex flex-wrap items-center justify-between gap-3 mb-6">
                <div>
                  ...
                  <p className="text-sm text-ink2">
                    Public page: {PARAMOTOR_HREFS[item.slug] || '—'}
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold mb-1">Card subtitle</label>
                  <input
                    value={item.listing.tagline}
                    onChange={(e) => updateField(item.id, 'tagline', e.target.value)}
                    className="w-full p-2 border rounded"
                    placeholder="Break the status quo"
                  />
                </div>
                <div>
                  <ImageUploadField
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full p-2 border rounded"
                    placeholder="Explore Disruptor"
                  />
                </div>
                <div className="md:col-span-2 border-t pt-4 mt-2">
                  <p className="text-sm font-bold text-ink mb-3">Comparison section</p>
