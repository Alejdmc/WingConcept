...
import {
  ...
  DISRUPTOR_TRIKE_INCLUDED,
  DISRUPTOR_TRIKE_FEATURES,
  DISRUPTOR_TRIKE_SPECS,
...
const CONFIGURATOR_HREF = '/paratrike/disruptor/configurador'
...
const disruptorFallback = {
  ...
  body: DISRUPTOR_TRIKE_SUMMARY.body,
  image: DISRUPTOR_TRIKE_HERO,
  year: 2026,
  brand: 'Wing Concept',
  philosophy: 'Conquer the Sky',
  basePrice: DISRUPTOR_TRIKE_BASE_PRICE,
  model: DISRUPTOR_TRIKE_SUMMARY.model,
  chassisSubtitle: DISRUPTOR_TRIKE_SUMMARY.subtitle,
  included: DISRUPTOR_TRIKE_INCLUDED,
  features: DISRUPTOR_TRIKE_FEATURES.map((f) => ({
    ...f,
    icon: ICON_MAP[f.icon] || Package,
  })),
  specs: DISRUPTOR_TRIKE_SPECS,
  gallery: DISRUPTOR_TRIKE_GALLERY,
}
...
export default function DisruptorTrikePage() {
  const [disruptor, setDisruptor] = useState(disruptorFallback)

  useEffect(() => {
    api.productos.obtener('disruptor-trike').then((p) => {
      ...
      setDisruptor((prev) => ({
        ...
        gallery: extra.gallery?.length
          ? extra.gallery.map((src, i) => ({ src, alt: `Disruptor Trike ${i + 1}` }))
          : DISRUPTOR_TRIKE_GALLERY,
      }))
    }).catch(() => {})
  }, [])

  const galleryItems = disruptor.gallery || DISRUPTOR_TRIKE_GALLERY
  const includedItems = disruptor.included || DISRUPTOR_TRIKE_INCLUDED
  ...
  return (
    <div className="min-h-screen bg-white">
            ...
            <motion.div
              ...
              <div>
                <p className="text-brand font-bold uppercase tracking-widest text-sm mb-4">
                  {disruptor.brand} — {disruptor.model || 'Trike Disruptor'} · {disruptor.year}
                </p>
        ...
        <div className="max-w-7xl mx-auto">
          <motion.div
            ...
            <p className="text-4xl sm:text-5xl font-black text-brand">
              ${Number(disruptor.basePrice ?? DISRUPTOR_TRIKE_BASE_PRICE).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              <span className="text-xl text-ink2 font-bold ml-2">USD</span>
            </p>
            <p className="text-ink2 mt-4 max-w-2xl mx-auto text-lg">
              {disruptor.chassisSubtitle || DISRUPTOR_TRIKE_SUMMARY.subtitle}. All characteristics below are included by default in every basic trike.
            </p>
          </motion.div>
          ...
          <motion.div
            ...
            <p className="text-xl sm:text-2xl font-black uppercase mb-6">
              Add harnesses & accessories in the configurator
            </p>
      ...
      <section className="py-24 px-6 bg-bg2">
        <div className="max-w-7xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} className="text-center mb-16">
            <h2 className="text-3xl sm:text-5xl font-black uppercase text-ink">Why Disruptor Trike?</h2>
          </motion.div>
      ...
      <Gallery images={galleryItems} eyebrow="Disruptor Trike" title="Photo Gallery" bgClass="bg-white" />
      ...
      <section className="py-24 px-6 bg-gradient-to-r from-brand to-brand/80">
        <div className="max-w-4xl mx-auto text-center text-white">
          ...
          <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-xl text-white/90 mb-12">
            Configure your Disruptor Trike and fly with real passive and active safety.
          </motion.p>
