...
import {
  findParamotorParaglider,
  PARAMOTOR_PARAGLIDERS,
  NO_PARAGLIDER_ID,
  paragliderDisplayName,
  resolveParagliderColorLabel,
} from '@/lib/paramotorParagliderOptions'
import {
  PARAMOTOR_COLOR_PRESETS,
  PARAMOTOR_CUSTOM_COLOR_ID,
  PARAMOTOR_CUSTOM_SURCHARGE,
...
export default function ConfiguratorDisruptorParamotorPage() {
  ...
  const { options, loading: optionsLoading, defaultSelections } = useConfigOptions(DISRUPTOR_PARAMOTOR_PRODUCTO_ID, DEFAULT_OPTIONS)
  const CONFIG_OPTIONS = {
    engines: options.engines,
    chassisFinishes: options.chassisFinishes?.length ? options.chassisFinishes : DEFAULT_OPTIONS.chassisFinishes,
  ...
  const [selectedColorId, setSelectedColorId] = useState(PARAMOTOR_COLOR_PRESETS[0].id)
  ...
  const [previewOption, setPreviewOption] = useState({
    id: `color-${PARAMOTOR_COLOR_PRESETS[0].id}`,
    image: `/images/disruptor/colors/${PARAMOTOR_COLOR_PRESETS[0].id}-1.jpg`,
  })
  ...
  } = useParagliderConfigurator({ findParaglider: findParamotorParaglider })
  ...
  const selectedAccessoryItems = CONFIG_OPTIONS.accessories.filter((a) => selectedUpgrades.includes(a.id))
  const colorLabel = resolveParamotorColorLabel(selectedColorId, customColorText)
  const cartBlocked = Boolean(engine?.priceTbd)

  const totalPrice = useMemo(() => {
    const base = DISRUPTOR_PARAMOTOR_BASE_PRICE
    const colorPrice = paramotorColorSurcharge(selectedColorId)
    ...
    return base + colorPrice + finishPrice + enginePrice + handPrice + propellerPrice + paragliderPrice + upgradesPrice
  }, [finish, engine, handThrottle, propeller, paragliderPrice, selectedUpgrades, CONFIG_OPTIONS.accessories, selectedColorId])
  ...
  const previewGallery = useMemo(() => {
    ...
    if (!previewOption?.id) return buildOptionGallery(null, null, PRODUCT_IMAGES)
    const optionId = previewOption.id.startsWith('color-') ? previewOption.id.replace('color-', '') : previewOption.id
    const primary = previewOption.image
      || (previewOption.id.startsWith('color-') ? `/images/disruptor/colors/${optionId}-1.jpg` : undefined)
    return buildOptionGallery(optionId, primary, PRODUCT_IMAGES, previewOption.gallery)
  }, [previewOption])

  const selectColorPreset = (color) => {
    setSelectedColorId(color.id)
    setPreviewOption({ id: `color-${color.id}`, image: `/images/disruptor/colors/${color.id}-1.jpg` })
  }

  const selectCustomColor = () => {
    setSelectedColorId(PARAMOTOR_CUSTOM_COLOR_ID)
    setPreviewOption({ id: 'color-custom', image: null })
  }
  ...
  useEffect(() => {
    switch (step) {
      case 0:
        setPreviewOption({
          id: selectedColorId === PARAMOTOR_CUSTOM_COLOR_ID ? 'color-custom' : `color-${selectedColorId}`,
          image: selectedColorId !== PARAMOTOR_CUSTOM_COLOR_ID
            ? `/images/disruptor/colors/${selectedColorId}-1.jpg`
            : null,
        })
        break
      case 1:
      ...
      case PARAGLIDER_STEP:
        if (paraglider) {
          ...
          setPreviewOption({ id: NO_PARAGLIDER_ID, image: null })
        }
        break
      case 6: {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      await addConfiguredProduct({
        ...
        customColor: selectedColorId === PARAMOTOR_CUSTOM_COLOR_ID ? customColorText.trim() : undefined,
          ...
          <motion.div initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
            <AnimatePresence mode="wait">
              ...
              >
                {step === 0 && (
                  <ChassisColorStep
                    title="Frame Color. Choose your finish"
                    presets={PARAMOTOR_COLOR_PRESETS}
                    customColorId={PARAMOTOR_CUSTOM_COLOR_ID}
                    customSurcharge={PARAMOTOR_CUSTOM_SURCHARGE}
                ...
                {step === PARAGLIDER_STEP && (
                  <ParagliderStep
                    ...
                    findParaglider={findParamotorParaglider}
                  ...
                  <ConfigSection title="Accessories. Enhance your setup">
                    <div className="space-y-3">
                      {CONFIG_OPTIONS.accessories.map((a) => (
                        <OptionCard
                          key={a.id}
                          selected={selectedUpgrades.includes(a.id)}
                ...
                {step === 7 && (
                  <ConfigSection title="Review & Purchase">
                    <div className="space-y-3 text-sm">
                      <SummaryRow label="Color" value={colorLabel} price={paramotorColorSurcharge(selectedColorId)} />
                    ...
                    {cartBlocked && (
                      <p className="mt-4 text-sm font-semibold text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-3">
                        The selected engine has a price to be confirmed. Choose a different engine or contact us to complete your order.
                      </p>
                    )}
                  </ConfigSection>
                )}
            ...
            {error && (
              <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="p-4 bg-red-100 text-red-700 rounded-lg text-sm font-semibold">
                {error}
              </motion.div>
            )}
            ...
            <motion.div layout className="bg-brand-soft border-2 border-brand rounded-xl p-6">
              ...
              <motion.p key={totalPrice} initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-4xl font-black text-brand">
            ...
            <div className="flex items-center justify-between gap-4">
              <button type="button" onClick={goPrev} disabled={step === 0} className="inline-flex items-center gap-2 px-5 py-3 rounded-lg border-2 border-borderline text-ink font-bold uppercase tracking-wide text-sm disabled:opacity-40 disabled:cursor-not-allowed hover:border-brand hover:text-brand transition-all">
                <ChevronLeft className="w-4 h-4" /> Previous
              </button>
              {step < STEPS.length - 1 ? (
                <button type="button" onClick={goNext} className="inline-flex items-center gap-2 px-5 py-3 rounded-lg bg-brand text-white font-bold uppercase tracking-wide text-sm hover:bg-brand/90 transition-all">
                  Next <ChevronRight className="w-4 h-4" />
                </button>
              ) : (
                <button type="button" onClick={handleAddToCart} disabled={loading || cartBlocked} className="inline-flex items-center gap-2 px-6 py-3 rounded-lg bg-brand text-white font-black uppercase tracking-wide text-sm hover:bg-brand/90 disabled:opacity-50 transition-all">
