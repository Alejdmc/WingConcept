...
if [ -s /tmp/safety_report.json ]; then
    ...
    if [ "$VULNERABILITIES" -gt "0" ]; then
        echo "⚠️  Se encontraron $VULNERABILITIES vulnerabilidades:"
        echo ""
...
if [ -f "../.gitignore" ]; then
    if grep -q "^\.env$" ../.gitignore; then
        ...
        echo "⚠️  .env NO está en .gitignore (riesgo de commit accidental)"
    fi
    ...
    echo "⚠️  .gitignore no encontrado en directorio raíz"
fi
