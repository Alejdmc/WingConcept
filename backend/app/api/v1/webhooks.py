...
async def webhook_wompi(
    ...
    Valida firma HMAC con WOMPI_EVENTS_SECRET.
    Docs: https://docs.wompi.co/docs/en/events

    Eventos principales:
    - transaction.updated: state_str APPROVED | DECLINED | VOIDED
    """
    payload_bytes = await request.body()
    payload_str = payload_bytes.decode("utf-8")

    # Validar firma HMAC
    # Docs: https://docs.wompi.co/docs/en/events#validar-firma
    ...
    if settings.WOMPI_EVENTS_SECRET and not hmac.compare_digest(firma_esperada, firma_header):
        logger.warning("Webhook Wompi: firma inválida")
    ...
    try:
        ...
        # Retornar 200 para que Wompi no reintente (el error se loggea)
...
async def webhook_stripe(
    ...
    if not stripe_signature:
        raise HTTPException(status_code=400, detail="Stripe-Signature header requerido")

    try:
        event = pago_service.validar_webhook_stripe(payload_bytes, stripe_signature)
    except ValueError as e:
        logger.warning(f"Webhook Stripe: firma inválida — {e}")
        raise HTTPException(status_code=401, detail="Firma Stripe inválida")
